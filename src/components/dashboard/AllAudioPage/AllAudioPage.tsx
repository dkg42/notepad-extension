...
function SortIndicator({ active, dir }: { active: boolean; dir: 'asc' | 'desc' }) {
  if (!active) return <span className="all-audio-sort-icon">&#x21C5;</span>;
  return (
    <span className="all-audio-sort-icon all-audio-sort-icon--active">
      {dir === 'asc' ? '▲' : '▼'}
    </span>
  );
}
    ...
    <div className="all-audio-page">
      ...
      {!isLoading && totalCount === 0 && (
        <div className="all-audio-page__empty">
          <span className="all-audio-page__empty-icon">&#x266B;</span>
